const API_BASE_URL = 'https://wydoraco-backend.netlify.app/api';
const DEFAULT_COMPANY_SLUG = 'shadow'; // Fallback se não for fornecido
...
window.onload = async function() {
    ...
    // Configurar o listener para Enter no input
    ...
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('chatHeader').innerText = `🤖 ${botConfig.botName || 'WydoraçoBot'}`;
            adicionarMensagem(`👋 Olá! Sou o assistente da ${botConfig.companyName || 'empresa'}. Como posso ajudar?`, "bot");
        } else {
...
async function processMessage(texto) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const botResponse = typeof data === 'string' ? data : (data.response || data.message || "Não entendi.");
